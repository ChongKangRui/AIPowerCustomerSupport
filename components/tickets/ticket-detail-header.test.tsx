...
// isAdmin/agents/onAssign/assignPending have their own admin-gating
// tests elsewhere. This file covers the plain customer/assignee
// fallback text. Every render below stays in the read-only "not an
// admin" branch unless a test overrides it.
...
// TicketDetailHeader is pure and presentational. The `ticket` prop
// drives it entirely, with no data fetching. That makes its fallback
// text (customer email alone versus "name · email", "Unassigned") cheap
// to cover directly, instead of through TicketDetailView plus a mocked
// network round trip.
//
// This mirrors tickets-table.test.tsx's coverage of the same fallback
// rules on the list page's Customer/Assigned columns.
describe("TicketDetailHeader", () => {
  ...
  // canAssign = isAdmin && ticket.status === OPEN
  // (ticket-detail-header.tsx). The real enforcement runs server-side
  // (app/api/tickets/[id]/assign/route.ts). This is the one place that
  // decides whether the control even renders. So each half of that
  // condition gets its own test case, instead of trusting the && to
  // keep working.
  describe("assign control", () => {
    ...
    it("picking an agent from the dropdown calls onAssign with that agent's id", async () => {
      ...
      // (see node_modules/@radix-ui/react-dropdown-menu). A plain
      // fireEvent.click never fires that handler in jsdom.
