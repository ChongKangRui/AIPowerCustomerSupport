...
- Demo login: `prisma/seed.ts` seeds one Admin + one Agent account (`SEED_ADMIN_*`/`SEED_AGENT_*` env vars, defaults documented in `.env.example`); the login page has "Demo agent"/"Demo admin" buttons that autofill those credentials via matching `NEXT_PUBLIC_SEED_*` vars. Intentionally left unguarded by `NODE_ENV` — this is a portfolio project, not a real business app, so a one-click demo login in production is acceptable.
