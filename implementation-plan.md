...
- [x] Manual test entrypoint `scripts/poll-gmail.ts` (`npm run gmail:poll`) since the cron route doesn't exist yet — verified against the real demo inbox: bootstrap, new-ticket creation, and the 404-skip path all confirmed working end-to-end. `lib/gmail.ts`'s "TEMP DEBUG" console logging has since been stripped.
- [x] Build `sendGmailReply()`: constructs a raw MIME message by hand (`In-Reply-To`/`References` headers set to the single most recent thread message, not a full accumulated chain — accepted simplification), sends via `gmail.users.messages.send` with `threadId`. Wired into a new `POST /api/tickets/[id]/reply` route + `hooks/use-send-ticket-reply.ts`, which is what the ticket detail page's previously-stubbed Reply box (`components/tickets/ticket-detail-view.tsx`) now actually calls — this also completes Phase 3's "Manual agent reply" item below.
- [x] Route Handler `app/api/cron/poll-gmail/route.ts` — checks `Authorization: Bearer $CRON_SECRET`, calls `pollGmailAndCreateTickets()`
- [x] Portfolio stand-in for a fast cron cadence: a manual "Fetch Gmail" button on the tickets list toolbar (`components/tickets/tickets-view.tsx`), any authenticated user, hitting a new session-authenticated `POST /api/gmail/poll` route (`hooks/use-sync-gmail.ts`) that calls the same `pollGmailAndCreateTickets()`. The real cron-job.org registration below still applies on top of this, just at a slower interval than a typical production setup.
...
- [x] Build Path A function: KB-stuffed confidence check, schema `{ confident: boolean, response: string }` — `lib/ai-auto-resolve.ts`'s `checkAutoResolve()`. Uses `generateText` + `Output.object` rather than the literal `generateObject` named here: this SDK version (`ai@7`) deprecates `generateObject` in favor of that (see AGENTS.md re: breaking changes vs. training data). Fails open (`confident: false`) on any model-call error, so a bad AI call routes to Path B instead of crashing the poll. Uses `geminiFlashLite()` (moved off `geminiFlash()` after this shipped — see `lib/gemini.ts`'s model split). That's a `-latest` alias, not a pinned snapshot like `gemini-2.5-flash`: a freshly created API key 404'd against that exact pinned model mid-testing ("no longer available to new users"), so both `lib/gemini.ts` exports use `-latest` aliases to avoid the same trap recurring.
...
- [x] In-app notification indicator (bell icon + unread badge + dropdown list, `components/notifications/notification-bell.tsx`, wired into `components/navbar.tsx`) — backed by a new `Notification` model (`prisma/schema.prisma`), `GET /api/notifications` (list + unread count) and `PATCH /api/notifications/[id]/read` / `PATCH /api/notifications/read-all`. `hooks/use-notifications.ts` polls every 20s (the first `refetchInterval` usage in this codebase — no push infra, mirrors the same "poll like Gmail ingestion already does" choice). A toast (newly added shadcn `sonner`, mounted in `app/providers.tsx`) flashes for anything new since the last poll, seeded silently on first load so an existing backlog doesn't toast-storm.
- [x] Wire it into the Phase 4 round-robin assignment step — plus, beyond what this checklist item literally named, the other two places a ticket's `assignedToId` changes: manual admin assignment (single `.../[id]/assign` and bulk `.../assign`) and the reopen-on-reply flow (`lib/gmail.ts`). Shared data builders in `lib/notifications.ts`. Each assignment write only notifies when the assignee actually changes (guards against a no-op re-PATCH) and never when an admin assigns a ticket to themselves; reopen always notifies, since "your resolved ticket came back as OPEN" is a distinct event from "you got assigned," even to the same agent.
- [x] Retention: `GET /api/notifications` opportunistically deletes the caller's own *read* notifications older than `NOTIFICATION_RETENTION_DAYS` (30, `models/notification.model.ts`) on every poll hit — no separate cron job, since cron-job.org isn't even wired up yet for Gmail polling (see below) and this table is already touched by the 20s poll anyway. Unread notifications are never pruned, at any age.
- [x] Cache sync: a genuinely new notification (detected client-side in `notification-bell.tsx`'s toast-dedup effect) also invalidates `["tickets"]` and `["ticket", ticketId]`, so a tickets list or ticket detail page that's already open doesn't keep serving stale cached data past what the notification just reported changed.
- [x] Coverage for a customer reply on an already-`OPEN`, already-assigned ticket (not just a reopen) — new `TICKET_NEW_MESSAGE` type, wired into `processMessage()`'s non-reopen branch in `lib/gmail.ts`. Skipped when the ticket has no assignee at all (Path B found no eligible agent) — same "no one to notify" guard as the assignment routes, rather than running round-robin on a reply.
- [x] Test coverage: `lib/notifications.test.ts` (the three data builders — pure, unit-tested directly) and `components/notifications/notification-bell.test.tsx` (badge/unread count incl. the "9+" cap, dropdown list rendering, mark-one/mark-all-read wiring, and a "toast flash" describe block that's regression coverage for the isLoading→isSuccess bug above — asserts silence on first load and exactly one toast for a genuinely new notification forced via a direct `["notifications"]` invalidation rather than waiting on the real 20s interval).
